...
_MMR_f32 CNDF_SIMD  (_MMR_f32 xInput ,unsigned long int gvl)
{
  ...
  xMask       = _MM_VFLT_f32_f(xInput,0.0,gvl);  //_MM_VFLT_f32(src2,src1)
  xInput      = _MM_VFSGNJX_f32(xInput,xInput,gvl); //_MM_VFSGNJX_f32(src2,src1)

  expValues   = _MM_MUL_f32(xInput, xInput,gvl);
  expValues   = _MM_MUL_f32_f(expValues, -0.5,gvl);

  xNPrimeofX = _MM_EXP_f32(expValues ,gvl);
  FENCE();
  xNPrimeofX = _MM_MUL_f32_f(xNPrimeofX, inv_sqrt_2xPI, gvl);
  ...
  xK2 = _MM_MUL_f32_f(xInput, 0.2316419, gvl);
  xK2 = _MM_ADD_f32_f(xK2,    1.0,       gvl);

  xK2   = _MM_RDIV_f32_f(1.0,xK2,gvl);
  ...
  xLocal_1 = _MM_MUL_f32_f(xK2,    0.319381530, gvl);
  xLocal_2 = _MM_MUL_f32_f(xK2_2, -0.356563782, gvl);

  xLocal_3 = _MM_MUL_f32_f(xK2_3, 1.781477937, gvl);
  ...
  xLocal_3 = _MM_MUL_f32_f(xK2_5, 1.330274429,gvl);
  xLocal_2 = _MM_ADD_f32(xLocal_2, xLocal_3,gvl);
  ...
  xLocal   = _MM_RSUB_f32_f(1.0, xLocal,gvl);
                          ...
                          fptype * time, int * otype/*,long int *  otype_d*/, float timet ,unsigned long int gvl)
{
    ...
    xfXd1   = _MM_MERGE_f32(_MM_RSUB_f32_f(1.0,xfXd1,gvl),xfXd1, xMask,gvl);
    xStockPrice = _MM_LOAD_f32(sptprice,gvl);
    xOptionPrice1 = _MM_MUL_f32(xStockPrice, xfXd1,gvl);
    FENCE();
    xfXd2   = _MM_MERGE_f32(_MM_RSUB_f32_f(1.0,xfXd2,gvl),xfXd2, xMask,gvl);
...
int bs_thread(void *tid_ptr) {
    ...
    unsigned long int gvl = __riscv_vsetvl_e32m8(end); //PLCT
    fptype* price;
...
#pragma omp parallel for private(i, price, priceDelta)
        ...
        for (i=start; i<end; i += gvl) {
            ...
            // equation.
            // gvl = __builtin_epi_vsetvl(end-i, __epi_e32, __epi_m8);
            gvl = __riscv_vsetvl_e32m8(end-i); //PLCT
